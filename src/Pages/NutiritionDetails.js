...
const NutiritionDetails = () => {
  ...
  const setFetchedData = () => {
    if(nutritionFetched.length > 1) {
       ...
       setNutritionData({
      calories:  nutritionFetched.filter(
        (data) => data.name === "Calories"
      )[0].amount,
      protein: nutritionFetched.filter(
        (data) => data.name === "Protein"
      )[0].amount,
      carbs: nutritionFetched.filter(
        (data) => data.name === "Net Carbohydrates"
      )[0].amount,
      sugar: nutritionFetched.filter(
        (data) => data.name === "Sugar"
      )[0].amount,
      fiber : nutritionFetched.filter(
        (data) => data.name === "Fiber"
      )[0].amount,
      fat : nutritionFetched.filter(
        (data) => data.name === "Fat"
      )[0].amount,
    });
    }
  ...
  return (
    <div className="nutrition-details-page">
      ...
      <nav className="NutritionScreen-btn">
        <Link to='/' className="go-back-btn"><i className="fa-solid fa-hand-point-left"></i></Link>
      </nav>

      <h1 className="nutrition-details-heading">Nutrition Details <span className="nutrition-details-heading-span">(Per serving)</span></h1>
      ...
      <div className="nutrition-details-page-nutrition-information">
        ...
        <ul>
          ...
          <li><i className="fa-solid fa-battery-three-quarters"></i> {nutritionData.calories ? `Calories ${nutritionData.calories} kcal`  :'calories'}</li>
          <li><i className="fa-solid fa-dna"></i> {nutritionData.protein ? `Protein ${nutritionData.protein} gram`  :'proteins'}</li>
          <li><i className="fa-solid fa-fire"></i> {nutritionData.carbs ? `Carbs ${nutritionData.carbs} gram`  :'carbs'}</li>
          <li><i className="fa-solid fa-cubes-stacked"></i> {nutritionData.sugar ? `Sugar ${nutritionData.sugar} gram`  :'sugar'}</li>
          <li><i className="fa-solid fa-cannabis"></i> {nutritionData.fiber ? `Fiber ${nutritionData.fiber} gram`  :'fiber'}</li>
          <li><i className="fa-solid fa-bottle-droplet"></i> {nutritionData.fat ? `Fat ${nutritionData.fat} gram`  :'fat'}</li>
        </ul>
      </div>
    </div>
  );
